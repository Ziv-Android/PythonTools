...
class OpenCVMask():
    def __init__(self):
        img_width = 512
        img_height = 512
        show = "../data/2018061301.jpg"
        hide = "../data/2018061302.jpg"
        size = img_width, img_height
        # read img
        self.img_show = cv2.resize(cv2.imread(show), size, interpolation=cv2.INTER_CUBIC)
        self.img_hide = cv2.resize(cv2.imread(hide), size, interpolation=cv2.INTER_CUBIC)
    ...
    def conv(self):
        # img = np.zeros((self.img_height, self.img_width, 4), np.uint8)
        # print(self.img_hide)

        alpha = 255 + self.img_hide - self.img_show
        print(alpha)
        data = np.clip((255 * (self.img_hide / (alpha + 1))), 0, 255)
        print(data)
        # img[:, :, :, 0] = data[:, :, :, 0]
        # img[:, :, :, 1] = img[:, :, :, 0]
        # img[:, :, :, 2] = img[:, :, :, 0]
        # data[:, :, :, 3] = alpha
        # print(data)
        cv2.namedWindow("conv_img", cv2.WINDOW_NORMAL)
        cv2.imshow("conv_img", data)
