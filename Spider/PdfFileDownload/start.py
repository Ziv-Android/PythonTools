...
host = "https://www.itf.gov.hk/l-sc/"
...
def down_load_pdf(url, name):
    result = requests.get(url, headers=headers)
    with open(name, "wb") as wf:
        for chunk in result.iter_content(chunk_size=1024):
            if chunk:
                wf.write(chunk)
    print("Download %s finish." % name)


if __name__ == "__main__":
    for source in source_list:
        file_url = host + source.get("filePath")
        school_name = code_to_school(source.get("uCode"))
        company_name = source.get("startUp").strip()
        file_name = school_name + "_" + company_name + ".pdf"
        print(file_url, file_name)
        down_load_pdf(file_url, file_name)
